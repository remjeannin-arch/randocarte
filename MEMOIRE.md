...
| v29-v30 | Diagnostics visibles partout ; **fix décisif panorama** : taille CSS explicite du canvas (sans elle, affichage à la taille interne ×dpr → seul le coin haut-gauche visible = « écran noir » iPhone dpr 3). Vérifié par capture d'écran locale : silhouette + étiquettes OK. Leçon : toujours fixer style.width/height d'un canvas plein écran |
| v31 | Sommets 2D/3D : **préchargement unique large (~150 km)** + rendu instantané depuis la mémoire (fin des lenteurs/disparitions liées aux requêtes Overpass à chaque déplacement) ; nouvelle requête seulement en sortant de la zone couverte |
| v32 | **Terrain HD IGN** (RGE ALTI ~5 m) : générateur BIL→terrarium (scratchpad build-demhd.js), zone Bavella embarquée (demhd/, 46 Mo), bouton HD en 3D avec **repli automatique** vers le terrain mondial ; fond Ortho IGN en 3D ; **base sommets+cols France embarquée** (peaks-fr.json, 65 530 entrées OSM, auto-installée en IDB, hors ligne) — fin de la dépendance Overpass à l'affichage. Manquent 3 morceaux de plaine nord-ouest (limites serveur), à compléter. NB : overpass.osm.ch = Suisse uniquement ! |
| v33 | Sommets/cols : **activés par défaut**, visibles dès le zoom 9 (15 plus hauts, puis densité croissante), ligne de diagnostic dans Options (taille de base, option, nombre affiché). Vérifié en local par capture : étiquettes sommets + cols OK en 3D à Bavella |
